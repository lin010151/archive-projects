...
public class InternshipActivity extends Activity implements IXListViewListener {
	...
	private XListView mInternshipListView; // 就业实习列表
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// 设置就业实习页面
		mInternshipListView = (XListView) findViewById(R.id.internshipListView);
		// 启用上拉加载，否则只能通过点击“加载更多”来加载。
		mInternshipListView.setPullLoadEnable(true);

		imgUtil = new ImageUtil();
		ImageView img = (ImageView) imgUtil.getImageView(this,
				R.drawable.internship);
		imgUtil.recycle();
		mInternshipListView.addHeaderView(img, null, false);
				...
				if (msg.what == 0x123) {
					if (!refreshormore) { // 刷新
						...
						madapter = new ArrayAdapter<String>(
								InternshipActivity.this, R.layout.list_item,
								NewsTitle);
						mInternshipListView.setAdapter(madapter);
					} else { // 装载更多
		...
		// 设置拉动监听。
		mInternshipListView.setXListViewListener(this);
		/** 设置新闻列表单行点击的监听。 */
		mInternshipListView.setOnItemClickListener(new OnItemClickListener() {
			@Override
	...
	private void onLoad() {
		...
		mInternshipListView.stopRefresh();
		mInternshipListView.stopLoadMore();
		// 此处要获取时间。
		mInternshipListView.setRefreshTime(curtime);
	}
