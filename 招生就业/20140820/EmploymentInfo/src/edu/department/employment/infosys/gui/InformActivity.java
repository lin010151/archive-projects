...
public class InformActivity extends Activity implements IXListViewListener {
	...
	private XListView mInformListView; // 事务通知列表
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		NewsTitle = new ArrayList<String>();// 用ArrayList<String>代替了string[]。
		...
		mInformListView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View view,
				...
				if (position > 1) { // 排除掉下拉加载更多行的监听事件,抬头图片与刷新提示占据了2行，故下标索引需要减掉2
			...
			public void run() {
				...
				try {
					...
					document = Jsoup.connect(
							getResources().getString(R.string.notice_url)
