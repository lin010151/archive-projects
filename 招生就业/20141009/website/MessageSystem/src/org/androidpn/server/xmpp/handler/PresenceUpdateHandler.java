		...
		try {
			...
			if (type == null) { // null == available
				...
				if (session != null) {
					...
					// 获取该用户的全部离线消息
					List<PushMessage> list = msgService
							.getPushMessageListByUsername(username);
					for (int index = 0; index < list.size(); index++) {
						PushMessage pushmsg = list.get(index);
						String msgsStr = pushmsg.getMessage();
						if (msgsStr != null) {
							// 获取消息
							JSONObject msg = JSONObject.fromObject(msgsStr);
							notificationManager.sendNotifcationToUser(
									msg.optString("apiKey"), username,
									msg.optString("title"),
									msg.optString("message"),
									msg.optString("uri"));
						}
						// 移除保存的消息
						msgService.removeMessage(pushmsg.getId());
					}
