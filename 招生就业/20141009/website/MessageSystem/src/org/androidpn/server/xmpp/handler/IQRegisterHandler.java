...
import edu.gdei.jobmessage.service.impl.ServiceLocator;
import edu.gdei.jobmessage.util.UserExistsException;
		...
		if (IQ.Type.get.equals(packet.getType())) {
			...
			try {
				...
				if (query.element("remove") != null) {
					...
					log.debug("usersave");
					User user = null;
					log.debug("session status=" + session.getStatus());
					if (session.getStatus() == Session.STATUS_AUTHENTICATED) {
						user = userService.getUserByUsername(session
								.getUsername());
						...
						log.debug("user name=" + username);
							if (user != null) {
								...
								log.debug("user created begin");
								user = new User();
								UuidUtil uuidUtil = new UuidUtil(); // Éú³ÉÖ÷¼üID
								user.setId(uuidUtil.getCode());
								user.setUsername(username);
								user.setPassword(password);
								user.setEmail(email);
								user.setName(name);
					...
					// log.debug("out id="+user.getId());
					// log.debug("out name="+user.getName());
					// log.debug("out password="+user.getPassword());
					// log.debug("out email="+user.getEmail());
					// log.debug("out updated_date="+user.getCreatedDate().toString());
					// log.debug("out username="+user.getUsername());
					// log.debug("usersaved");
