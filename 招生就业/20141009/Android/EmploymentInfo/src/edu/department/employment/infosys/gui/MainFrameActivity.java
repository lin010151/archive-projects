...
import org.androidpn.client.ServiceUtil;
...
public class MainFrameActivity extends ActivityGroup implements OnClickListener {
	...
	private String[] items = { "修改密码", "忘记密码", "个人信息", "标题效果", "页面效果", "退出登录", "推送设定","关于" };
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mLoginBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				intent.setClass(MainFrameActivity.this, LoginActivity.class);
				startActivityForResult(intent, 0);				// 设置当前Activity为1
		...
		seteffectPopupWindow
				.setOnPopupWindowClickListener(effectChangeListener);
		// 设置弹出菜单
		...
		preferences = getSharedPreferences("Login", context.MODE_WORLD_WRITEABLE);
		String loginname = preferences.getString("name", "");
		if (!loginname.isEmpty()) {
			// 找到登录按钮
			mLogin_Btn.setText(loginname);
			mLogin_Btn.setClickable(false);
			// 设置内存数据
			Student student = new Student();
			student.setStuname(loginname);
			student.setEmail(preferences.getString("email", ""));
			GlobalDataInstance.GetInstance().setStudent(student);
			Login login = new Login();
			login.setUserID(preferences.getString("userID", null));
			login.setUserPSW(preferences.getString("userPSW", null));
			login.setQuestion(preferences.getString("question", null));
			login.setAnswer(preferences.getString("answer", null));
			// 保存登录信息
			GlobalDataInstance.GetInstance().setLogin(login);
			// 设置登录成功
			synchronized (GlobalDataInstance.GetInstance().getLoginlock())
			{
				GlobalDataInstance.GetInstance().getLoginlock().setLock(true);
				GlobalDataInstance.GetInstance().setLoginsuccess(true);
				GlobalDataInstance.GetInstance().getLoginlock().notifyAll();
			}

			// 启动Androidpn服务，接收官方推送消息
			startAndroidpnServer();
		}
	}
	...
	private void initeViews() {
		...
		mSelectedItem.setWidth((getScreenWidth() - DimensionUtility.dip2px(
				this, 20)) / 4);
	...
	OnPopupWindowClickListener settingChangeListener = new OnPopupWindowClickListener() {
		...
		public void onPopupWindowItemClick(int position) {
			...
			case 0:
				// 修改密码
				if (!GlobalDataInstance.GetInstance().getLogin().getUserID()
						.isEmpty()) {
					Intent changepswintent = new Intent();
					changepswintent.setClass(MainFrameActivity.this,
							ChangePSWActivity.class);
					startActivityForResult(changepswintent, 1);				// 设置当前Activity为1
					overridePendingTransition(R.anim.in_drop, R.anim.out_fade);
				}
				break;
			case 1:
					Intent takebackintent = new Intent();
					takebackintent.setClass(MainFrameActivity.this,
							ObtainPSWActivity.class);
					startActivityForResult(takebackintent, 2);
					overridePendingTransition(R.anim.in_drop, R.anim.out_fade);
				break;
			case 2:
				// 学生详细信息
				if (!GlobalDataInstance.GetInstance().getLogin().getUserID()
					...
					infointent.setClass(MainFrameActivity.this,
			...
			case 3:
				// 弹出设置标题特效子菜单
				seteffectPopupWindow.showAtLocation(
						findViewById(R.id.setting_btn), Gravity.LEFT,
						getScreenWidth() - seteffectPopupWindow.getWidth(), 2);
				setefft = 0;
				break;
			case 4:
				// 弹出设置页面特效子菜单
			...
			case 5:
				...
				if (GlobalDataInstance.GetInstance().isLoginsuccess()) {
					// 设置登录按钮
					mLogin_Btn.setText("登录");
					mLogin_Btn.setClickable(true);
					// 设置登录共用变量
					GlobalDataInstance.GetInstance().setLoginsuccess(false);
					GlobalDataInstance.GetInstance().setLogin(null);
					// 记录退出登录情况
			...
			case 6:
				// 如果已登录，则可设置消息推送设定
				if (GlobalDataInstance.GetInstance().isLoginsuccess()) {
					ServiceManager.viewNotificationSettings(MainFrameActivity.this);
				}
				break;
			case 7:
				// 弹出关于窗口
				Intent aboutintent = new Intent();
				aboutintent.setClass(MainFrameActivity.this,
						AboutActivity.class);
	...
	public void setTitleSelected() {
		...
		case 1:
			// 动画滑动
			ImageAnimation.SetImageSlide(mSelectedItem, startX, mItemWidth, 0,
					0);
			// 设置滑动后动画开始位置
			startX = mItemWidth;
			// 设置选中项显示文字，也就是高亮部分文字
		...
		case 2:
			// 动画滑动
			ImageAnimation.SetImageSlide(mSelectedItem, startX, mItemWidth * 2,
					0, 0);
			// 设置滑动后动画开始位置
			startX = mItemWidth * 2;
			// 设置选中项显示文字，也就是高亮部分文字
		...
		case 3:
			// 动画滑动
			ImageAnimation.SetImageSlide(mSelectedItem, startX, mItemWidth * 3,
					0, 0);
			// 设置滑动后动画开始位置
			startX = mItemWidth * 3;
			// 设置选中项显示文字，也就是高亮部分文字
		...
		if (currentActivity instanceof InformActivity) {
			Log.v("currentActivity=", "InformActivity");
		}
		if (currentActivity instanceof InternshipActivity) {
			Log.v("currentActivity=", "InternshipActivity");
		}
		if (currentActivity instanceof RecruitmentActivity) {
			Log.v("currentActivity=", "RecruitmentActivity");
		}
		if (currentActivity instanceof OrganisesActivity) {
			Log.v("currentActivity=", "OrganisesActivity");
		}
	}
	...
	protected void onActivityResult(int requestCode, int resultCode,
		...
		if (requestCode == 0 && resultCode == 0) {
			...
			// 取出Bundle中的数据
			if (data.getBoolean("loginresult")) {		// 登录成功
				mLogin_Btn.setText(GlobalDataInstance.GetInstance()
						.getStudent().getStuname());
				mLogin_Btn.setClickable(false);
				// 通知内部招聘信息模块更新
				synchronized (GlobalDataInstance.GetInstance().getLoginlock())
				{
				...
				// 清除保存的androidpn登录信息
				serviceManager.clearsharedPrefs();
				// 启动Androidpn服务，接收官方推送
				startAndroidpnServer();
			}
		}
		if (requestCode == 1 && resultCode == 1)	// 修改密码
		{
			// 取出Intent里的Extras数据
			Bundle data = intent.getExtras();
			// 取出Bundle中的数据
			if (data.getBoolean("changepassword")) {
				// 提示用户，然后返回上一层
		...
		if (requestCode == 2 && resultCode == 2)	// 找回密码
		{
			// 取出Intent里的Extras数据
			Bundle data = intent.getExtras();
			// 取出Bundle中的数据
			if (data.getBoolean("obtainpassword")) {
				// 提示用户，然后返回上一层
				AlertDialog.Builder builder = new AlertDialog.Builder(
						MainFrameActivity.this);
				// 设置对话框标题
				builder.setTitle("取回密码成功")
				// 设置对话框图标
						.setIcon(R.drawable.gdei_logo)
						// 设置对话框消息内容
						.setMessage("您的密码是: "+data.getString("psw")+"，\n请记住密码。")
						// 添加确定按钮
						.setPositiveButton("确定", null)
						// 显示
						.create().show();
    ...
    private void startAndroidpnServer(){
        ...
        RunningServiceInfo runningServiceInfo = ServiceUtil.isServiceRunning(MainFrameActivity.this, "org.androidpn.client.NotificationService");
